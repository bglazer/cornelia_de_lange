...
pV, input_weights = model(starts)
velocity = Vgpu[idxs]
original_loss = mse(pV, velocity)
print(f'Original loss: {original_loss.item():.9f}')

with torch.no_grad():
    min_weight = torch.log10(torch.min(torch.abs(input_weights)))
    max_weight = torch.log10(torch.max(torch.abs(input_weights)))
