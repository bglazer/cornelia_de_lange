...
import numpy as np
import scanpy as sc
from flow_model import LinearFlowModel
...
#%%
if len(sys.argv) < 2:
    print('Usage: python train_linear_flow_model.py <timestamp>')
    sys.exit(1)
tmstp = sys.argv[1]
...
model = LinearFlowModel(input_dim=num_nodes, 
                        predict_var=True)

#%%
def train(model_idx, gpu):
    logfile = open(f'{outdir}/logs/linear_flow_model_{model_idx}_{genotype}.log', 'w')
    ...
    for epoch in range(n_epoch+1):
        ...
        l1_loss = torch.sum(torch.abs(node_model.linear.weight))
        loss = mean_loss + var_loss + l1_loss
        ...
        if epoch % checkpoint_interval == 0:
            ...
            val_loss = val_mean_loss + val_var_loss
...
trained_models = parallel(delayed(train)(i, i%num_gpus) for i in range(num_nodes))

model_filename = f'{outdir}/models/linear_flow_model_{genotype}.pickle'
